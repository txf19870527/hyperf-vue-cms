...
class AuthMiddleware implements MiddlewareInterface
{
    ...
    protected function validate(ServerRequestInterface $request):ServerRequestInterface
    {
        ...
        if (
            empty($token) &&
            (
                !in_array_UpLow($uri, config("auth.no_check_uri"))
                || in_array_UpLow($uri, config("auth.must_login_uri"))
            )
        ) {
        ...
        if (empty($userData['id'])) {
            ...
            if (in_array_UpLow($uri, config("auth.no_check_uri")) && !in_array_UpLow($uri, config("auth.must_login_uri"))) {
                return $request;
            } else {
                throw new BusinessException(ResponseCode::LOGIN_TIME_OUT);
            }
        ...
        // 无需权限验证的接口，绕过权限验证
        if ( in_array_UpLow($uri, config("auth.no_check_uri")) ) {
            return $request;
        }

        // 超级管理员，绕过权限验证
        if (in_array($userData['id'], config("auth.super_admin_id"))) {
            return $request;
        }
