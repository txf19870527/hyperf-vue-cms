...
class PermissionService implements AbstractServiceInterface
{
     ...
     * @return array|mixed
     */
    public function getPermissionDropDown($depth = 2, $status = -1)
    {
        $type = range(1, $depth);
    ...
    public function lists($params)
    {
        ...
        $qb = $qb->where(function ($qb) use ($params) {
            ...
            if (!empty($params['uri'])) {
                $qb->where("uri", "like", "%{$params['uri']}%");
            }

            if (!empty($params['index'])) {
                $qb->where("index", "like", "%{$params['index']}%");
            }
